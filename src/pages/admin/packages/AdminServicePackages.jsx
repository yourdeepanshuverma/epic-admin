...
import { useGetAdminServicePackagesQuery, useUpdateServicePackageStatusMutation } from "../../../store/api/adminApi";
...
const AdminServicePackages = ({ status }) => {
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState("");
  const [confirmDialog, setConfirmDialog] = useState({ open: false, id: null, approved: false });
  
  const { data, isLoading, isFetching } = useGetAdminServicePackagesQuery({
    page,
    limit: 10,
  ...
  const [updateStatus, { isLoading: isUpdating }] = useUpdateServicePackageStatusMutation();
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold capitalize">{status || "All"} Service Packages</h2>
        <Input 
            ...
            {isLoading ? (
              ...
              packages.map((pkg) => (
                <TableRow key={pkg._id}>
                  ...
                  <TableCell>{pkg.serviceSubCategory?.name}</TableCell>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      <Button size="icon" variant="ghost" asChild>
                         <a href={`${import.meta.env.VITE_SITE_URL}/service/${pkg.slug}`} target="_blank" rel="noreferrer">
