...
import { IconPlus, IconTrash, IconPhoto, IconX } from "@tabler/icons-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const ServicePhotosTab = ({ pkg }) => {
  const [addAlbum, { isLoading: isAdding }] = useAddServiceAlbumMutation();
  const [deleteAlbum] = useDeleteServiceAlbumMutation();
  
  // Manage Album Mutations
  const [addPhotos, { isLoading: isAddingPhotos }] = useAddServiceAlbumPhotosMutation();
  const [deletePhoto] = useDeleteServiceAlbumPhotoMutation();
  const [updateTitle] = useUpdateServiceAlbumTitleMutation();
...
export default ServicePhotosTab;
