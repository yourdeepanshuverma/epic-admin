...
import { useGetServicePackagesQuery, useDeleteServicePackageMutation, useUpdateServiceStatusMutation } from "../../store/api/vendorApi";
...
const MyServicePackages = () => {
  const { data, isLoading } = useGetServicePackagesQuery();
  const [deletePackage, { isLoading: isDeleting }] = useDeleteServicePackageMutation();
      ...
      cell: ({ row }) => (
        <div className="flex gap-2">
          <Button variant="ghost" size="icon" asChild>
            <Link to={`/my-packages/services/edit/${row.original._id}`}>
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold">My Service Packages</h2>
        <Button asChild>
      ...
      <AlertDialog open={!!packageToDelete} onOpenChange={(open) => !open && setPackageToDelete(null)}>
        <AlertDialogContent>
            ...
            <AlertDialogDescription>
                This will permanently delete the service package from your listings. This action cannot be reversed.
            </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
...
export default MyServicePackages;
